...
object L1ValueVariableTypes extends App {
  ...
  val functionList: List[(Int, Int) => Int] = List(addFunction, multiplyFunction)
  val inputList = List((1, 2), (3, 4), (5, 6), (7, 8))
  val partialFunction: PartialFunction[(Int, Int), List[Int]] = {
    case (a, b) => functionList.map(_(a, b))
  }
  val outputMapList = inputList.map {
    partialFunction
  }
  val outputFlatMapList = inputList.flatMap {
    partialFunction
  }
  // You get a List with 2 elements which are pointers to a lambda function (addNew, multiplyNew)
  println(functionList)
  // Each element of inputList will return a List
  println(outputMapList)
  // We flatten the inner elements to get one outer List
  println(outputFlatMapList)

  val y = (a: Int) => a * 2 // Inferred type of Int => Int
  println(y)
  println(y(2)) // think in terms of pointer, like a pointer to a function

  val x: Int = 42
  println(x)

  // VALs ARE IMMUTABLE
  // vals are like to const in C, but they are not used in different parts of the code
  ...
  println(aString.flatMap(_.toString + " "))
  ...
  //semicolon can be used at the end, but not necessary
  ...
  // The side effects let us see what our program is doing
